...
export async function renderOgImage() {
  ...
  // Load fonts (Milonga for the display title, Geist for supporting copy).
  const [milonga, geistBold, geistRegular] = await Promise.all([
    loadGoogleFont('Milonga', 400, title),
    loadGoogleFont('Geist', 700, `${eyebrow}${location}`),
    loadGoogleFont('Geist', 400, subtitle),
  ])

  const fonts = [
    milonga && { name: 'Milonga', data: milonga, weight: 400 as const, style: 'normal' as const },
    ...
    (
      ...
      >
        ...
        >
          ...
          <div
            style={{
              fontFamily: 'Milonga, Geist, serif',
              fontSize: 132,
              lineHeight: 1,
              color: '#ffffff',
              whiteSpace: 'nowrap',
