...
export async function renderOgImage() {
  ...
  // Load fonts (Ewert for the display title, Geist for supporting copy).
  const [ewert, geistBold, geistRegular] = await Promise.all([
    loadGoogleFont('Ewert', 400, title),
    loadGoogleFont('Geist', 700, `${eyebrow}${location}`),
    loadGoogleFont('Geist', 400, subtitle),
  ])

  const fonts = [
    ewert && { name: 'Ewert', data: ewert, weight: 400 as const, style: 'normal' as const },
